...
class SignUpViewModel extends GetxController {
  ...
  var password1FieldIcon = const Icon(
    Icons.visibility_off, color: Colors.white,).obs;
  var password2FieldIcon = const Icon(
    Icons.visibility_off, color: Colors.white,).obs;

  password1VisibilityFunc() {
    if (passwd1Visibility.value == true) {
      passwd1Visibility.value = false;
      password1FieldIcon.value =
    ...
    try {
      ...
      if (methods.isEmpty) {
        ...
        if (newUser.runtimeType == UserCredential) {
          final ref = FirebaseStorage.instance.ref().child('user_docs').child(
              newUser.user!.uid).child('DP');
          ...
          Uint8List imageBytes = await io.File(imageFile.value!.path)
              .readAsBytes();
      ...
      if (e.code == 'user-already-exists') {
        ...
        overlay.value = false;
        showToast(
            'Servers cannot be reached at the moment due to Network Error.\nCheck your Internet connection and try again');
      }
      else {
        overlay.value = false;
